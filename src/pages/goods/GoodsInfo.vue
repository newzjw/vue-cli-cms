    ...
    <div class="mui-card">
      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          ...
          <!-- 把getSelectedCount方法传递给子主件，:max父组件给子组件传值 -->
...
export default {
  ...
  methods: {
    getLunbotu () {
      axios.get('http://www.liulongbin.top:3005/api/getthumimages/' + this.id).then(result => {
        if (result.data.status === 0) {
          // 先循环轮播图数组的每一项，为 item 添加 img 属性，因为 公用的轮播图 组件中，只认识 item.img， 不认识 item.src
    ...
    enter (el, done) {
      ...
      // done()
      // 小球动画优化思路：
      // 1. 先分析导致动画不准确的本质原因：小球最终位移到的位置，已经局限在了某一分辨率下的滚动条未滚动的情况下；
