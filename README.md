...
5. 当发表评论OK后，重新刷新列表，以查看最新的评论
 + 如果调用 getComments 方法重新刷新评论列表的话，可能只能得到 最后一页的评论，前几页的评论获取不到（pageIndex不为1）
 + 换一种思路： 当评论成功后，在客户端，手动拼接出一个 最新的评论对象，然后 调用 数组的 unshift 方法， 把最新的评论，追加到  data 中 comments 的开头；这样，就能 完美实现刷新评论列表的需求；

## 改造图片分享 按钮为 路由的链接并显示对应的组件页面

## 绘制 图片列表 组件页面结构并美化样式
 1. 制作 顶部的滑动条（mui的tab-top-webview-main）
 2. 制作 底部的图片列表
### 制作顶部滑动条的坑们：
 1. 需要借助于 MUI 中的 tab-top-webview-main.html 
 2. 需要把 slider 区域的 mui-fullscreen全屏 类去掉，不然样式有问题
...
4. total control手机共屏软件

## 商品详情页面
1. 创建goodsinfo,GoodsDesc,GoodsComment并在router.js中引入
2. 这里使用 window.location.href 的形式进行跳转，而不使用a标签。这里需要参考vue-router的文档，编程式导航
3. 使用MUI的card组件构建商品详情页面
4. 轮播图，抽离首页的轮播图组件swiper.vue，注意要解决轮播图中图片的宽度问题
5. 使用编程式导航实现图文介绍和商品评论的跳转
6. 加入购物车的动画效果,注意要适配不同滚动条位置和分辨率
7. 实现购买数量的变化，当numbox的值发生变化，立刻把购买数量传递给父组件，需要用到父子组件的传值，watch
8. 使用js 的api设置numbox的最大值，max是异步获取的，出现undefined问题，使用到watch解决。查看MUI的number box文档

## vuex
1. vuex 是 Vue 配套的 公共数据管理工具，它可以把一些共享的数据，保存到 vuex 中，方便 整个程序中的任何组件直接获取或修改我们的公共数据；
2. 面试题：data，props，vuex的区别。data是保存私有数据，props是保存父组件传递过来的数据，vuex是保存需要共享的数据

## 购物车的实现
1. 安装vuex并引入
2. 在index.js中定义方法addToCar,功能：点击加入购物车，把商品信息，保存到 store 中的 car 上
3. 在goodsinfo组件的addToShopCar方法里拼接goodsinfo，调用 store 中的 mutations 来将商品加入购物车
4. 使用getters实现徽标的商品数量更新
5. 实现商品的本地持久存储
6. 绘制购物车内容页，获取购物车内的所有商品信息并加装到该页面
